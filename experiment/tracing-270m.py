...
# and written to circuit_tracing_results_270m.json in the next section.
...
# to circuit_tracing_results_270m.json.
...
try:
    ...
    with open("circuit-tracer/experiment/tracing/circuit_tracing_results_270m.json", "w") as f:
        json.dump(output_data_with_downstream, f, indent=2)
     
    print("\nCombined results successfully saved to tracing/circuit_tracing_results_270m.json")

except Exception as e:
    print(f"\nERROR: Failed to write JSON file: {e}")
    # Fallback: save what we have so we don't lose the whole script's progress
    with open("circuit-tracer/experiment/tracing/circuit_tracing_results_270m_fallback.json", "w") as f:
