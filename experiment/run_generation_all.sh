#!/usr/bin/env bash
# Run generation scripts sequentially, saving a timestamped log per stage.
# Usage:
#   bash run_generation_all.sh            # runs 270m, 1b, 4b in order
#   bash run_generation_all.sh 4b         # runs only 4b
#   bash run_generation_all.sh 1b 4b      # runs 1b then 4b
# For a long unattended run, launch inside tmux, or prefix with nohup:
#   nohup bash run_generation_all.sh > run_all.log 2>&1 &
...
declare -A SCRIPTS=(
    [270m]="generation-gemma-3-270m.py"
    [1b]="generation-gemma-3-1b.py"
    [4b]="generation-gemma-3-4b.py"
)
...
run_stage() {
    local name="$1"
    local script="$2"
    local log="$LOGDIR/gen-${name}_${TS}.log"
